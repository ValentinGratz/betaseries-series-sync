...
async function request<T>(path: string, options: RequestOptions = {}): Promise<T> {
  ...
  if (!response.ok || (json.errors && json.errors.length > 0)) {
    ...
    const baseMessage = firstError?.text ?? `BetaSeries API error (HTTP ${response.status})`;
    const suffix = firstError?.code ? ` [code ${firstError.code}]` : ` [HTTP ${response.status}]`;
    throw new BetaSeriesApiError(`${baseMessage}${suffix}`, response.status, firstError?.code);
  }
