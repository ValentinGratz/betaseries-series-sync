...
(() => {
  ...
  // src/content/m6plus/detector.ts
  var EMBEDDED_JSON_VARS = ["__NUXT__", "__6PLAY_STATE__", "__INITIAL_STATE__"];
  function detectM6PlusEpisode(doc = document, url = location.href) {
  ...
  function tryExtractFromEmbedded(state, url) {
    const candidate = deepFind(state, (node) => {
      return node && typeof node === "object" && (typeof node.season === "number" || typeof node.numeroSaison === "number") && (typeof node.episode === "number" || typeof node.numeroEpisode === "number");
    });
  ...
  // src/content/m6plus/history.ts
  function scanM6PlusHistory(doc = document) {
  ...
  // src/content/m6plus/content-script.ts
